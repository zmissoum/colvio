...
- **Cross-org compare (DEV vs PROD drift)** — export a solution's component list as a **compare file** (.json), load it on another org and diff against the counterpart solution: same three buckets plus a version-drift note; two-pass matching stated on screen (GUIDs for solution-transported components, unambiguous type+name for metadata whose MetadataIds are org-local)
...
- Filter everything by **security role or business unit** — with a "+ child BUs" option widening the BU filter to its whole subtree (parent BUs selectable even without direct users) — instant, client-side; needs "Audit user access" enabled, sees what your audit retention keeps (both stated in the UI)
