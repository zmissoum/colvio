...
System Ops
System jobs monitor (find stuck workflows, bulk cancel/resume with the documented state transitions), plug-in trace viewer (exceptions highlighted, durations, quick time windows matched to the platform's ~24h purge, CSV export), and the Power Automate run history of solution cloud flows read from the org-side flowrun table (~28-day retention) — status badges with a Failed filter, duration, trigger, error messages, export. Everything paginates without a hard cap and adds server-side filters, so nothing stays hidden behind a Top-N limit.
