...
const COMMON_STATS = (L) => [
  ...
  [L.stats3, "79"],
  [L.stats4, "260 (Vitest)"],
  [L.stats5, "~905 KB (+ ~430 KB xlsx, ~373 KB pptxgen " + L.onDemand + ")"],
...
const FR = {
  ...
  s1: "1. Vue d'ensemble",
  s1p1: "Colvio est une extension Chrome **Manifest V3** (React 18 + Vite) pour Microsoft Dynamics 365 / Dataverse. Elle fournit 19 modules d'exploration, de chargement et d'audit, sans aucun backend : toutes les requêtes partent du navigateur de l'utilisateur vers sa propre org, authentifiées par la session existante.",
  ...
  s2aT: "Composants",
  s2comp: [
    "`panel` (src/) — l'application React : 38 composants, un module = un onglet. Le bridge (`d365-bridge.js`) expose l'API typée, gère le rate-limit (30 req/s), le cache et les pools de workers des opérations bulk. La logique à risque (préparation typée des écritures, filtres OData, historique) vit dans des modules purs testés (`updateUtils`, `filterUtils`, `historyUtils`, `loaderUtils`…).",
    "`background.js` — service worker : relais de messages panel ↔ content script **résilient à la mise en veille des onglets** (l'onglet D365 est épinglé non-endormissable, réveillé/ré-injecté au besoin, IDs remplacés suivis via tabs.onReplaced) avec **garde d'origine** : une requête ne part jamais vers un onglet affichant un autre environnement. Rejette tout message dont `sender.id` n'est pas l'extension.",
    "`content.js` — injecté sur `*.dynamics.com` : extraction du contexte org, **79 actions** (query, metadata, batch…), validation des entrées (regex noms d'entités/champs, GUIDs, strip des caractères de contrôle), construction des $batch, back-off 429.",
  ],
  s2bT: "Protocole de messages",
  s2msg: "Requête : `{__d365InspectorRequest, id, action, params, d365TabId, orgUrl}` ; réponse : `{result}` ou `{error}`. Timeouts côté bridge : 30 s standard, 10 min pour les opérations bulk. Les chunks bulk sont plafonnés à 500 enregistrements par message pour rester sous la limite IPC de 64 Mo de Chrome.",
  s2msgb: [
  ...
  modRows: [
    ["Data Explorer", "4 modes de requête (Builder/OData/FetchXML/SQL), filtres relationnels (nav-path parent + lambdas any()/not any()), pagination auto, virtual scrolling + barre horizontale collante, édition inline et bulk update TYPÉS par les métadonnées (lookups via @odata.bind), détecteur de doublons, bulk delete $batch avec ✕ Cancel, historique restaurant les requêtes Builder dans le Builder (valeurs expurgées — vie privée)."],
    ["SQL → FetchXML", "Parseur dédié (`sqlToFetchXml.js`, 43 tests) : SELECT/JOIN/WHERE/GROUP BY/agrégats → link-entity, attributs, filtres."],
    ["API Tester", "Action `customRequest` : méthode whitelistée, hôte re-validé deux fois, timeout 60 s, réponse brute (statut/headers/body) non interprétée, téléchargement .json/.txt."],
    ["Data Loader", "Voir section 5 — parsing, transforms, 4 modes, moteur $batch, coercition typée par métadonnées, auto-resume après timeout."],
    ["Show All Data / Metadata", "EntityDefinitions + attributs ; édition inline typée des champs ; gestionnaire de BPF ; exports CSV dictionnaire de données et OptionSets."],
    ["Automation / Apps", "Inventaire statique des plug-in steps + processus (classification Microsoft/Managed/Custom) ; inventaire des apps model-driven (inférence include-all, badges EXPLICIT/IMPLICIT, inspecteur de vues, subgrids, recherche inversée)."],
    ["Env Variables", "Défauts + overrides typés (yes/no, JSON validé, nombres), piège NO VALUE compté, secrets = références Key Vault."],
    ["Schema (ERD) / Relationships / Schema diff", "Rendu SVG custom (drag/zoom/pan), exports PNG 2x / SVG / Mermaid ; snapshots JSON + diff classé entre orgs."],
    ["Solutions / Translations", "Composants résolus (40+ types), comparaison intra-org et cross-org (fichier de compare, matching 2 passes) ; labels multilingues + export/import zip solution-wide (ExportTranslation)."],
    ["Recycle Bin", "FetchXML datasource='bin' + action Restore ; détection d'activation ; limitations MS affichées."],
    ["Audit History / System Ops", "audits + RetrieveAuditDetails ; asyncoperation (cancel/resume bulk), traces plug-ins (fenêtres rapides ~24 h), runs des cloud flows de solution (table flowrun)."],
    ...
    ["Business Units / Adoption", "Hiérarchie + organigramme plein écran (PNG), déplacement d'utilisateurs en masse (vérité sur les rôles annoncée), sélection par liste collée ; analytics d'adoption (agrégation serveur par jour, DAU/WAU/MAU, comptes de service séparés, rapport PowerPoint natif)."],
  ],
  ...
  s6b: [
    ...
    "**Écritures typées centralisées** (`updateUtils`, testé) : toute valeur saisie est convertie selon le type réel du champ et refusée lisiblement avant envoi — un texte dans un GUID/lookup ou un nombre mal saisi ne part jamais au serveur (et ne vide jamais un champ).",
    "**Garde d'origine du relais** : le panel déclare son org sur chaque requête ; le service worker refuse tout onglet affichant un autre environnement.",
  ],
  s7: "7. Performance",
  s7b: [
...
const EN = {
  ...
  s1: "1. Overview",
  s1p1: "Colvio is a **Manifest V3** Chrome extension (React 18 + Vite) for Microsoft Dynamics 365 / Dataverse. It ships 19 exploration, loading and audit modules with no backend at all: every request goes from the user's browser to their own org, authenticated by the existing session.",
  ...
  s2aT: "Components",
  s2comp: [
    "`panel` (src/) — the React app: 38 components, one module = one tab. The bridge (`d365-bridge.js`) exposes the typed API and owns rate limiting (30 req/s), caching, and the bulk worker pools. Risky logic (typed write preparation, OData filters, history) lives in pure tested modules (`updateUtils`, `filterUtils`, `historyUtils`, `loaderUtils`…).",
    "`background.js` — service worker: panel ↔ content-script relay, **resilient to browser tab sleep** (the D365 tab is pinned non-discardable, woken/re-injected as needed, replaced ids tracked via tabs.onReplaced) with an **origin guard**: a request never goes to a tab showing a different environment. Rejects any message whose `sender.id` isn't the extension.",
    "`content.js` — injected on `*.dynamics.com`: org context extraction, **79 actions** (query, metadata, batch…), input validation (entity/field regexes, GUIDs, control-char stripping), $batch building, 429 back-off.",
  ],
  s2bT: "Message protocol",
  s2msg: "Request: `{__d365InspectorRequest, id, action, params, d365TabId, orgUrl}`; response: `{result}` or `{error}`. Bridge timeouts: 30 s standard, 10 min for bulk operations. Bulk chunks are capped at 500 records per message to stay under Chrome's 64 MB IPC limit.",
  s2msgb: [
  ...
  modRows: [
    ...
    ["API Tester", "`customRequest` action: whitelisted methods, host validated twice, 60 s timeout, raw response (status/headers/body) returned unmodified, .json/.txt download."],
    ["Data Loader", "See section 5 — parsing, transforms, 4 modes, $batch engine, metadata-typed coercion, auto-resume after timeout."],
    ["Show All Data / Metadata", "EntityDefinitions + attributes; typed inline field editing; BPF manager; data-dictionary and OptionSet CSV exports."],
    ["Automation / Apps", "Static inventory of plug-in steps + processes (Microsoft/Managed/Custom classification); model-driven app inventory (include-all inference, EXPLICIT/IMPLICIT badges, view inspector, subgrids, reverse search)."],
    ["Env Variables", "Defaults + typed overrides (yes/no, validated JSON, numbers), NO VALUE trap counted, secrets = Key Vault references."],
    ["Schema (ERD) / Relationships / Schema diff", "Custom SVG rendering (drag/zoom/pan), PNG 2x / SVG / Mermaid exports; JSON snapshots + ranked cross-org diff."],
    ["Solutions / Translations", "Components resolved (40+ types), intra-org and cross-org compare (compare file, two-pass matching); multilingual labels + solution-wide zip export/import (ExportTranslation)."],
    ["Recycle Bin", "FetchXML datasource='bin' + Restore action; enablement detection; MS limitations surfaced."],
    ["Audit History / System Ops", "audits + RetrieveAuditDetails; asyncoperation (bulk cancel/resume), plug-in traces (quick ~24 h windows), solution cloud-flow runs (flowrun table)."],
    ...
    ["Business Units / Adoption", "Hierarchy + full-screen org chart (PNG), bulk user move (roles truth stated first), paste-a-list selection; adoption analytics (server-side per-day aggregation, DAU/WAU/MAU, service accounts separated, native PowerPoint report)."],
  ],
  ...
  s6b: [
    ...
    "**Centralized typed writes** (`updateUtils`, tested): every typed-in value is converted by the field's real type and refused readably before sending — text in a GUID/lookup or a mistyped number never reaches the server (and never silently clears a field).",
    "**Relay origin guard**: the panel states its org on every request; the service worker refuses any tab showing a different environment.",
  ],
  s7: "7. Performance",
  s7b: [
